      ...
      <main id="main" className="pb-[84px] lg:pb-10">
        <PageHero trail={trail}>
          <RecordHero
            logo={e.logo}
            name={e.name}
            badge={e.security.lastBreachYear === null
              ? { text: 'No breach', tone: 'strong' }
              : { text: `Breach ${e.security.lastBreachYear}`, tone: 'plain' }}
            rank={r.rank}
            of={all.length}
            score={r.score.total}
            ...
            facts={[
              { label: 'Founded', value: String(e.founded) },
              { label: 'Headquarters', value: countryName(e.headquarters), flag: e.headquarters },
              { label: 'Taker fee', value: `${e.takerFeePct}%` },
              { label: '24h spot volume', value: volumeBand(e.spotVolumeUsd) },
            ]}
        ...
        <div className="shell pt-3 sm:pt-[13px] lg:pt-4 flex flex-col gap-0 sm:gap-[13px] lg:gap-4">
          ...
          <div className="split rail-left">
            ...
            <Card className="p-4 lg:p-6" as="section" id="security">
