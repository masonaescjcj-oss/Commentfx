...
/**
 * The bar that takes the header's place on the way down.
 *
 * A record page is long, and the header a reader has scrolled past is not the
 * thing they need while they are in the middle of it — the page's own sections
 * are. So the two swap: scroll down and the header slides up and this slides
 * into the space it left; scroll up and they swap back. Neither is ever absent,
 * which is the whole point of the pair.
 *
 * Fixed rather than sticky, and translated out of view rather than hidden, so
 * it costs the document no height at any scroll position and cannot shift a
 * single pixel of the page. It is exactly --hdr-h tall, because it is standing
 * in the header's footprint.
 *
 * The same items as the grid above, deliberately. Two lists of a page's
 * sections that could disagree is two lists to maintain.
 */
export function SectionBar({ name, items }: {
  name: string;
  items: Array<{ href: string; label: string; ready?: boolean }>;
}) {
  return (
    <nav aria-label="Sections of this page" className="section-bar">
      <div className="shell flex items-center gap-3 h-[var(--hdr-h)]">
        <span className="font-[family-name:var(--font-display)] font-bold text-[14px] tracking-[-0.025em] shrink-0 max-w-[34vw] truncate">
          {name}
        </span>
        {/* Scrolls sideways rather than wrapping: a bar that grows to two rows
            on a long name is a bar that moves the page under it. */}
        <ul className="flex items-center gap-1 overflow-x-auto no-bar -mx-1 px-1">
          {items.filter((i) => i.ready !== false).map((i) => (
            <li key={i.href}>
              <a href={i.href} className="block whitespace-nowrap px-[10px] py-[6px] rounded-[9px] text-[13px] text-ink-2 hover:bg-card-2 hover:text-ink">
                {i.label}
              </a>
            </li>
          ))}
